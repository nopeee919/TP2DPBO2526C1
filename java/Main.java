...
public class Main {
  ...
  private static ArrayList<MobilRental> daftarMobil = new ArrayList<>();
  ...
  private static void tampilkanTabel() {
    ...
    String format = "| %-6s | %-13s | %-6s | %-15s | %-5s | %-10s | %-10s | %-10s | %-10s | %-15s |%n";
  ...
  private static String formatRupiah(double nilai) {
    ...
    long angka = (long) nilai;
    String hasil = String.valueOf(angka);

    int posisi = hasil.length() - 3;

    while (posisi > 0) {
      hasil = hasil.substring(0, posisi)
          + "."
          + hasil.substring(posisi);
